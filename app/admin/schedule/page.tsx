...
import { deleteScheduleItem } from "@/app/admin/actions";
...
export default async function AdminSchedulePage() {
  ...
  const [items, classes] = await Promise.all([
    prisma.scheduleItem.findMany({ orderBy: [{ displayOrder: "asc" }, { startTime: "asc" }] }),
  ...
  return (
    <AdminShell>
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-3xl font-extrabold text-brand-navy">Цагийн хуваарь</h1>
        <Link
          ...
          <table className="w-full text-left text-sm text-brand-navy">
            <thead className="bg-brand-mist/50 text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-5 py-4 font-bold">Гарчиг</th>
                <th className="px-5 py-4 font-bold">Байршил</th>
                <th className="px-5 py-4 font-bold">Анги</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-slate-500">
                    Одоогоор хуваарь бүртгэгдээгүй байна.
                  </td>
                </tr>
              ) : null}
              {items.map((item) => (
                <tr key={item.id} className="transition hover:bg-slate-50">
                  <td className="px-5 py-4 font-bold text-brand-blue whitespace-nowrap">
                  ...
                  <td className="px-5 py-4">
                    {item.classId ? classMap[item.classId] : "-"}
                  </td>
