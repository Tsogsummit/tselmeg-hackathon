...
const pages = Array.from({ length: 15 }, (_, index) => {
  ...
  return {
    src: `/slides/2b/page-${page}.png`,
    label: `${index + 1}-р хуудас`,
  };
});

export default function TwoBPage() {
  return (
    <PublicShell showBack>
      <section className="mb-6 overflow-hidden rounded-2xl bg-brand-navy text-white shadow-soft">
        <div className="relative p-6 sm:p-10">
          ...
          <h1 className="text-5xl font-black tracking-tight sm:text-7xl">2Б</h1>
        ...
        {pages.map((page) => (
          <figure key={page.src} className="overflow-hidden rounded-2xl border border-brand-line bg-white shadow-soft">
            ...
            <div className="bg-slate-100 p-2 sm:p-4">
              <Image
                src={page.src}
                alt={`2Б ангийн тайлан ${page.label}`}
                width={1400}
                height={1980}
                className="mx-auto h-auto w-full max-w-5xl rotate-180 scale-x-[-1] rounded-lg bg-white shadow-sm"
